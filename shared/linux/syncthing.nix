...
let
  otherDevices = builtins.filter (device: device != machine);
in
{
  options = {
  ...
  config = lib.mkIf config.syncthing.enable {
    services.syncthing = {
      ...
      settings = {
        gui =
          let
            password = builtins.readFile ../secrets/syncthing-password.key;
          in
          {
            inherit user password;
          };

        devices = builtins.removeAttrs {
          "storm" = {
