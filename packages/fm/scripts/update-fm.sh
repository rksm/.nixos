...
tag=$(curl -fsSL https://api.github.com/repos/cboone/fm/releases/latest | jq -er .tag_name)
version=${tag#v}
if [[ "$current" == "$version" ]]; then
  echo "fm $version is current."
  exit 0
fi
[[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]

binary_hash=$(nix store prefetch-file --json \
  "https://github.com/cboone/fm/releases/download/$tag/fm_${version}_linux_amd64.tar.gz" | jq -er .hash)
skill_hash=$(nix store prefetch-file --json --unpack \
  "https://github.com/cboone/fm/archive/refs/tags/$tag.tar.gz" | jq -er .hash)
